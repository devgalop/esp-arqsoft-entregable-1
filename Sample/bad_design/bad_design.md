...
| **Alto Acoplamiento** | Las clases están fuertemente acopladas entre sí, lo que significa que un cambio en una clase puede tener un impacto significativo en las demás. Por ejemplo, si se modifica la clase `Producto`, esto podría requerir cambios en `DetalleFactura` y posiblemente en `Factura`. |
